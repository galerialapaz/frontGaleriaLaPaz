	...
	return function() {
		...
		1: [function(require, module, exports) {
			...
			function createPanZoom(domElement, options) {
				...
				function pause() {	releaseEvents();	paused = true	}
				function resume() {
					if (paused) {	listenForEvents();	paused = false	}
				}
				...
				function setZoomSpeed(newSpeed) {
					if (!Number.isFinite(newSpeed)) {	throw new Error("Zoom speed should be a number")	}
					speed = newSpeed
				}
				function getPoint() {
...
/*Zomm Pad*/
function zoomPadInit(id){
	let el = document.getElementById(id)
	let pz = storageJsonPre("padzoom",{x:5,y:110,scale:0.99})
	let z = pz.scale
	let x = pz.x
	let y = pz.y
	var inst
	if(z<1){
	  inst = panzoom(el, {
		...
		initialX: x/(1-z),initialY: y/(1-z),initialZoom: z,
		maxZoom: 25, minZoom: 0.99
	  })
	}else{
	  inst = panzoom(el, {
		zoomDoubleClickSpeed: 1,
		initialX: -x/(z-1),initialY: -y/(z-1),initialZoom: z,
		maxZoom: 25, minZoom: 0.99
	  })
	}
	inst.on('pan',()=>{ localStorage.setItem("padzoom",JSON.stringify(inst.getTransform()) ) });
	inst.on('zoom',()=>{ localStorage.setItem("padzoom",JSON.stringify(inst.getTransform()) ) });
  }
/*Zomm Pad*/
/*doble click nicho*/
var dbClikTim = true
...
let id1 
let id2
function nichoClikPanzoom(e){
  if(dbClikTim) {
    dbClikTim = false;
    id1 = e.target.id
    clikTim = setTimeout(()=>{ dbClikTim = true}, 700);
  } else {
    id2 = e.target.id
    clearTimeout(clikTim);
    dbClikTim = true;
    if(id1==id2){
      return id2
    }else{
...
/*doble click nicho*/
function resetMapa(){
  localStorage.setItem("padzoom",JSON.stringify({x:5,y:110,scale:0.99}) )
